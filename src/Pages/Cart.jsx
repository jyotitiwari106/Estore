...
import { assets, dummyAddress } from '../Assest/assets'
        ...
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                <h1 className="text-3xl font-medium mb-6">
                    Shopping Cart <span className="text-sm text-green-500">{getCartCount()}</span>
                </h1>
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        <div className="flex items-center md:gap-6 gap-3">
                            <div onClick = {()=>{
                                navigate(`/product/${product.category.toLoweCase()}/${product._id}`); window.scrollTo(0, 0);
                ...
                <button onClick={()=> {navigate('/products'); window.scrollTo(0,0)}}className="group cursor-pointer flex items-center mt-8 gap-2 text-green-500 font-medium">
                   <img className='group-hover:translate-x-1 transition' src={assets.arrow_right_icon_colored} alt="Array" />
                    Continue Shopping
                </button>
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        ...
                        {showAddress && (
                           ...
                           {addresses.map ((address, index)=>(
                            <p onClick={() =>{selectedAddress(address); setShowAddress(false)}} className="text-gray-500 p-2 hover:bg-gray-100">
