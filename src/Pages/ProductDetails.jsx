...
const ProductDetails = () => {
  ...
  return (
    <div className="max-w-6xl flex flex-col w-full px-6">
      <p>
        <Link to={'/'}>Home</Link> /
      ...
      <div className="flex flex-col md:flex-row gap-16 mt-4">
       ...
       <div className='flex flex-col items-center'>
            ...
            <div className='grid grid-col-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-6 mt-6 w-max'>
        {relatedProducts.filter((product)=> product.inStock).map((product, index)=>(
          <ProductCard key={index} product={product}/>
        ))}
        </div>
        <button onClick={()=>{navigate('/products'); window.scrollTo(0, 0);}} className='border border-green-500 p-2 cursor-pointer px-12 py-2.5 my-16 rounded text-green-400 hover:bg-green-500/10 transition'>See more</button>
        </div>
